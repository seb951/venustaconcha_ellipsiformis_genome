...
gff = read.table("annotation_quast_v3/predicted_genes/assembly_k41_B24G_H4_kc3-10_glimmer_genes.gff",stringsAsFactors=F)
gff_modified = gff[,1]
#for(i in 1:nrow(gff))
#{
...
system("head -1 annotation_quast_v3/predicted_genes/assembly_k41_B24G_H4_kc3-10_glimmer_genes.gff >head")
system("cat head temp >annotation_quast_v3/predicted_genes/assembly_k41_B24G_H4_kc3-10_glimmer_genes2.gff")
system("rm head temp")


#get the sequences in fasta format
if(file.exists("assembly_k41_B24G_H4_kc3-10_rr_CDS.fa")==F) system("bedtools getfasta -s -fi assembly_k41_B24G_H4_kc3-10_rr.fa -bed annotation_quast_v3/predicted_genes/assembly_k41_B24G_H4_kc3-10_glimmer_genes2.gff -fo temp_fasta 2>log")

#modify format to get rid of ":"
if(file.exists("assembly_k41_B24G_H4_kc3-10_rr_CDS.fa")==F) system("awk '{gsub(/:/,\"_\")}; 1' temp_fasta >assembly_k41_B24G_H4_kc3-10_rr_CDS.fa")
...
write.table(gff_annotated,"annotation_quast_v3/predicted_genes/assembly_k41_B24G_H4_kc3-10_glimmer_genes_annotated_uniprot.gff",col.names = T, row.names = F, quote = T,sep = "\t")
...
write.table(gff_annotated,"annotation_quast_v3/predicted_genes/assembly_k41_B24G_H4_kc3-10_glimmer_genes_annotated_trembl.gff",col.names = T, row.names = F, quote = T,sep = "\t")
